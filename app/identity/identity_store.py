...
from app.identity.schemas import FaceEmbedding, IdentityProfile, IdentityStoreEntry
...
class IdentityStore:
    ...
    def save(
        ...
        embedding_matrix = np.asarray([item.embedding for item in embeddings], dtype=np.float32)
        embedding_matrix = self._l2_normalize_rows(embedding_matrix)
        np.save(person_dir / "embeddings.npy", embedding_matrix)

        now = utc_now_iso()
        profile = IdentityProfile(
            person_id=safe_person_id,
            person_name=person_name,
            embedding_count=int(embedding_matrix.shape[0]),
    ...
    def list(self) -> list[IdentityStoreEntry]:
        entries: list[IdentityStoreEntry] = []
        for profile_path in sorted(self.root_dir.glob("*/profile.json")):
            try:
                data = json.loads(profile_path.read_text(encoding="utf-8"))
                entries.append(IdentityStoreEntry(**data))
            except Exception:
                continue
        return entries
